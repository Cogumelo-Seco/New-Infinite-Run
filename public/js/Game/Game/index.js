...
module.exports = (scene, camera, renderer, materials, Listener, data) => {
    ...
    const run = () => {
        ...
        let interval = setInterval(() => {
            if (state.player.dead) return clearInterval(interval)
            
            const couter = document.getElementById('couter')
            if (Number(couter.innerText) > 1) couter.innerText = Number(couter.innerText)-1
            ...
            if (!Listener.state.paused) {
                ...
                if (state.cubeBirthSpeed <= 160 && data.difficulty == 3) state.cubeSpeed = 0.5
                else {
                    if (state.cubeBirthSpeed <= 160) state.cubeBirthSpeed = 200
                    state.cubeSpeed = state.cubeBirthSpeed/(state.cubeBirthSpeed-150)
                }          
